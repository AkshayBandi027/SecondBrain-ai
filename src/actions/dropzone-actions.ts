...
export const extractText = async (url: string,fileName: string,fileType: string) => {
 ...
 const processedDocs = splitDocs.map((doc: {pageContent: string,metadata: {loc: {pageNumber: number}}}) => ({
 ...
 const vectorEmbeddings =  await Promise.all(processedDocs.map(async(doc: {pageContent: string, metadata: {pageNumber: number}}) => {
